...
<div>
    ...
    <table width="100%" border="0.5"
        style="font-weight: bold; font-size: 12pt; border-collapse: collapse;margin-top: 5px;" cellpadding="2">
        <thead>
        ...
        <tbody>
            @foreach ($box->castes as $key => $cast)
                <tr
                    @if ($key % 2 == 0) style="background-color: rgb(255, 255, 255)"
                    @else
                    style="background-color: rgb(238, 237, 237)" @endif>
                    <td width="3%" style="text-align:center">{{ $cast->number }}</td>
                    <td width="15%" style="text-align:center;text-transform: uppercase;">
                        {{ Str::upper($cast->mark) }}</td>
